<template>
	...
	<block v-if="data">
	<scroll-view style="height: calc(100vh);" scroll-x="false" scroll-y="true">
		...
		<view>
			...
			<xt-panal-list :count="data.length">
				...
				<view v-for="(item,index) of data" slot="card{{index}}">
					 <view style="display:flex;" class="zcard" @click="openHotel(item.hotel_id[0])">
						<view style="width:160px;min-height:120px">
							<image style="width:100%;height:100%" mode="aspectFill" :src="item.hotel_id[0].firstImages" />
						</view>
						<view style="padding:10px" class="zcard-right">
							<view class="item"><text class="title">{{item.hotel_id[0].hotelName}}</text></view>
							<view class="item"><text>距你直线距离 <text style="padding:0 4px;font-weight:bold">{{getDistance(location ,item.hotel_id[0].hotelCoordinate)}}</text>KM</text></view>
							<view class="item">
								<view class="tabscontainer" style=""> 
									<uni-tag v-for="i of item.hotel_id[0].feature" :inverted="true" :text="i" size="mini"/>
								</view>

							</view>
							<view class="item"><text style="color:#a1a1a1">{{item.hotel_id[0].hotelAddress}}</text></view>
