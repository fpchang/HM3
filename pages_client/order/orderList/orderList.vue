<template>
	<view class="orderList">
		...
		<unicloud-db ref="udb" v-slot:default="{data, loading, error, options}" :collection="colList" :getone="false"
			:where="_WHERE" orderby="createTime desc">
				...
				<xt-panal-list :count="data.length">
					...
					<view v-for="(item,index) of data" slot="card{{index}}">
						<view class="p-card">
							...
							<view class="info">
								<text>{{formatDateLabel(item.checkInStartDate)}}至{{formatDateLabel(item.checkInEndDate)}}</text><text
									style="padding:0 15px"><text>{{item.roomTypeArray.length}}</text>间{{dayNum([item.checkInStartDateTimeStamp,item.checkInEndDateTimeStamp])}}晚</text><text>大床房</text>
							</view>
							<view class="price">
							...
							<view v-if="item.orderType=='bargain'&&item.orderStatus==4" class="pay-area">
								...
								<view v-if="item.hotel_id[0].payType!='online'">
									<button size="default" type="default" class="btn"
										@click="confirmOrder(item)">确认</button>
								</view>
								<view>
	...
	export default {
		...
		setup() {
			...
			let colList = computed(() => {
				...
				return [
					db.collection('hm-order').where(
						`fromClient==true && createrPhone=='${store.state.user.phone}'`).getTemp(),
