  ...
  <view class="orderDishes content">
    <view class="flex-center top-area">
      <view style="background: #fff; padding: 5px; width: 140px">
        <uni-datetime-picker
          v-model="orderDishesForm.mealDate"
          type="date"
          @change="dateConfim"
        >
          <view class="flex-center">
            <uni-icons type="calendar" size="24" color="#60626680"></uni-icons
            ><text
              :style="{
                color: orderDishesForm.mealDate ? '#313131' : '#60626680',
              }"
              :start="Date.now()"
              >{{ orderDishesForm.mealDate || "请选择用餐时间" }}</text
            >
          </view>
        </uni-datetime-picker>
      </view>
        ...
        <uv-vtabs-item :index="index">
          <view class="item" :key="index2">
            ...
            <view class="item-content menuDetail">
              <view
                v-for="i of item._id['hm-menuDetail']"
                class="menuDetail-item"
              >
                <view class="l">
                  <image
                    src="https://env-00jxh1m2dpmq.normal.cloudstatic.cn/HM/images/food.jpg"
                    style="width: 80px; height: 60px"
                  ></image>
                </view>
                <view class="r">
                  <view style="flex: 1"
                    ><text style="padding-left: 15px">{{ i.name }}</text></view
                  >
				  <view style="flex:1;font-size:12px;color:#6464649c;padding:5px 0 0 25px"><text>{{i.mark}}</text></view>
                  <view class="r-item">
                    <view class="price-style"
                      ><text>￥</text><text>{{ i.price }}</text>
                    </view>
    ...
    <view style="height: 60px; position: relative" class="bottom-panal">
      <view
        class="bottom-panal-detail"
        v-if="goodsListPanalHeight != '0'"
          ...
          <view class="list-item" v-for="item of checkMenuList">
            ...
            <view class="ad-lose-num flex-center">
              <uni-icons
                ...
                size="24"
                color="#ED9121"
                @click="loseMenuCount(item)"
                v-if="item.checkCount > 0"
              ></uni-icons>
          ...
          <view style="display: flex; align-items: center">
            <view style="position: relative">
              <uni-icons
                type="cart-filled"
                size="30"
                :color="checMenuCount ? '#ED9121' : '#616161'"
              >
              </uni-icons>
              <uni-badge
                :text="checMenuCount"
            ...
            <text style="color: red; font-weight: bold; padding-left: 12px"
              >￥{{ checMenuPriceTotal }}</text
            >
            <text style="color: #ed9121" v-if="checMenuPriceTotal > 0"
              >*（不包含餐具等其它费用）</text
            >
          </view>
          <!-- <view> 
