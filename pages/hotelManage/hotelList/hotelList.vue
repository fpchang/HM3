              ...
              <view class="card-content">
                <view class="avator">
                  <image
                    ...
                    mode="aspectFill"
                    :src="
                      transformImageUrl(item.firstImages)
                ...
                <view class="info">
                  ...
                  <view class="n"
                    ><text>{{ `店主：${item.belong}` }}</text>
                  </view>
                  <view class="r"
                    ><text style="padding-right: 6px">{{
                      `角色：${this.roleFormat(item.curRole)}`
                    }}</text></view
                  >
                </view>
                <view class="control">
                  ...
                  <view
                    ...
                    ></l-icon
                  ></view>
                  <view class="control-item" @click="viewDetail(item)"
                    ><l-icon
                      name="fluent:more-circle-24-filled"
                      color="#36D399"
                      size="30px"
                    ></l-icon>
                  </view>
...
export default {
  ...
  methods: {
    transformImageUrl(url = "") {
      if (!url) {
        return 'https://cdn.hotelsys.fun/HM/images/app.png';
      }
      return url.replace(
        "https://env-00jxhfhjd231.normal.cloudstatic.cn",
        "https://cdn.hotelsys.fun"
      );
    },
    getSubtitle(item) {
      let str = "";
      return `【店主：${item.belong}】【角色：${this.roleFormat(
        item.curRole
      )}】`;
    },
    roleFormat(val) {
    ...
    editHotel(targetObj) {
      ...
      uni.navigateTo({
        url: `/pages/hotelManage/createHotel/createHotel?type=${
          this.type
    ...
    async deleteHotel(targetObj) {
      ...
      HotelService.deleteHotel(targetObj._id)
        .then(async (res) => {
          //console.log("删除成功");
          await this.$store.dispatch("getHotelList");
