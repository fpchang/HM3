					...
					<view :class="['top-area', isSticky? 'sticky-style':'']">
						<view class="title-area" :style="{opacity: opacityVal}">
							...
							<view class="check-area" @click="showCheckHotel">
								<text class="$uni-font-size-lg h-n-style">{{hotel.hotelName}}</text>
						...
						<view class="navbar">
							<view class="nav-content">
								<uv-tabs @click="clickTab" :list="tabList" lineWidth="0" lineColor="#f56c6c"
					...
					<scroll-view class="swiper-container-scroll" :scroll-y="true" :scroll-x="false"
						...
						<view v-if="dataHasRead">
							...
							<scenicSpotListComponent :key="item.time" :createTime="item.time"
							 ...
							 <hotelList :key="item.time" :createTime="item.time" v-if="item.ComponentName=='hotelList'">
							</hotelList>
						</view>
					</scroll-view>
				</swiper-item>
						...
						<view class="card">
							<view class="list-panal">
								<view class="list-panal-item" v-for="item of hotelList" @click="checkHotel(item._id)">
									<uni-icons :type="item._id == hotel_id?'checkbox':'circle'" size="22"
										color="#06c"></uni-icons>
									<view class="i-area">
