  ...
  <view>
    <uni-forms ref="baseForm" :modelValue="orderForm" label-width="80px">
      ...
      <uni-forms-item label="房型房间" required>
            ...
            >
              ...
              <view
                ><uni-number-box
                  ...
                  :min="1"
                  :max="item.remainRoomList.length"
        ...
        <!-- <view class="room-list-style">
					<view class="flex-start dev-area"><view>自动匹配房间</view><view><switch checked color="#FFCC33" style="transform:scale(0.7)"/></view></view>
					<view class="room-list"></view>
				</view> -->
      </uni-forms-item>
	...
	computed: {
		...
		roomTypeArray() {
			...
			return list.map((item) => {
				//自动装载房间
				let autoSelectRoomList = item.remainRoomList.filter((is,index)=>index<item.selectCount);
				let autoSelectRoomIdList =autoSelectRoomList.map(i=>{
					return i._id;
				})
				return {
